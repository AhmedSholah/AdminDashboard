...
const addOrder = async (req, res) => {
  try {
    const { orderId, orderDate, status, customer, shippingAddress, paymentInfo, products } = req.body;

    if (!orderId || !orderDate || !status || !customer || !shippingAddress || !paymentInfo || !products) {
    ...
    await newOrder.save();
    const populatedOrder = await Order.findById(newOrder._id).populate('customer').populate('products.productId');
  ...
  try {
    ...
    for (let newProduct of products) {
      ...
      for (let existingProduct of order.products) {
        if (existingProduct.productId.toString() === newProduct.productId.toString()) {
    ...
    for (let product of order.products) {
      const existingProduct = await Product.findById(mongoose.Types.ObjectId(product.productId)); // Ensure ObjectId conversion
      if (existingProduct) {
        totalPrice += existingProduct.price * product.quantity;
      }
    }
    ...
    const populatedOrder = await updatedOrder.populate('products.productId');
...
const getOrderById = async (req, res) => {
  ...
  try {
    const order = await Order.findOne({ orderId: id }).populate("customer").populate("products.productId");
    if (!order) {
      return res.status(404).json({ message: "Order not found" });
    }
